...
class ScrollbarUltima extends StatefulWidget {
  ...
  late final Widget Function(BuildContext context, Animation<double> animation, Set<WidgetState> widgetStates) _thumbBuilder;
  Widget Function(BuildContext context, Animation<double> animation, Set<WidgetState> widgetStates) get thumbBuilder => _thumbBuilder;
  ...
  final Widget Function(BuildContext context, Animation<double> animation, Set<WidgetState> widgetStates)? trackBuilder;

  late final Widget Function(
      BuildContext context, Animation<double> animation, Set<WidgetState> widgetStates, double offset, int? precalculatedIndex)? _labelBuilder;
  Widget Function(BuildContext context, Animation<double> animation, Set<WidgetState> widgetStates, double offset, int? precalculatedIndex)?
      get labelBuilder => _labelBuilder;
  ...
  ScrollbarUltima(
    ...
    _thumbBuilder = thumbBuilder ??
        (context, thumbAnimation, widgetStates) => createDeffaultThumb(
            context, thumbAnimation, widgetStates, backgroundColor, defaultThumbThickness, minDynamicThumbLength, scrollbarPosition);
    checkAsserts();
  }
...
class ScrollbarUltimaState extends State<ScrollbarUltima> with TickerProviderStateMixin {
  ...
  bool get isVerticalScroll => widget.scrollbarPosition == ScrollbarPosition.right || widget.scrollbarPosition == ScrollbarPosition.left;
  ...
  double? get _maxScrollOffset => (widget.maxScrollOffset ??
      (_scrollController != null ? _scrollController!.position.maxScrollExtent - (widget.maxScrollOfssetFromEnd ?? 0) : null));
  ...
  void initState() {
    ...
    _thumbController = ShowHideAnimatedController(mustBeHidden: _thumbMustBeHidden, durationBeforePlannedHidding: widget.durationBeforeHide);
    _labelController = ShowHideAnimatedController(mustBeHidden: _labelMustBeHidden, durationBeforePlannedHidding: widget.durationBeforeHide);
    _trackController = ShowHideAnimatedController(mustBeHidden: _trackMustBeHidden, durationBeforePlannedHidding: widget.durationBeforeHide);
                            ...
                            ScrollbarPositionedWithAlign.fill(
                                ...
                                child: RepaintBoundary(child: Builder(builder: (context) {
                                  final showHideAnimated = ShowHideAnimated(
                                      ...
                                      builder: (context, animation) => widget.trackBuilder!(context, animation, materialStates));
                                    ...
                                    child: ShowHideAnimated(
                                        ...
                                        builder: (context, animation) => Builder(builder: (context) {
                                              ...
                                              return widget.labelBuilder!(
                                                  context, animation, materialStates, _actualScrollViewOffset, _precalculateIndex());
                                            })))),
                          ScrollbarPositionedWithAlign(
                              position: widget.scrollbarPosition,
                                  ...
                                  final thumb = Builder(builder: (context) {
                                    ...
                                    final double? dynamicLength = widget.dynamicThumbLength
                                        ? (calculateDynamicThumbLength() ?? 0)
                                            .clamp(widget.minDynamicThumbLength ?? 0, widget.maxDynamicThumbLength ?? double.infinity)
                                        : null;
  ...
  double? _calculateThumbOffset() {
    ...
    final double thumbOffset = (relativeScrollbarOffset / scrollbarScrollRange * _thumbScrollRange).clamp(0, _thumbScrollRange);
    return thumbOffset;
  }
  ...
  void _onThumbDragged(DragUpdateDetails details) {
    final newOffset =
        _addDragDeltaToOffset(oldOffset: _actualScrollViewOffset, dragDelta: details.primaryDelta!, thumbScrollRange: _thumbScrollRange);
    _actualScrollViewOffset = newOffset;

    moveScrollViewToOffset(newOffset);
  }

  double _addDragDeltaToOffset({required double oldOffset, required double dragDelta, required double thumbScrollRange}) {
    final absoluteDelta = dragDelta / thumbScrollRange;
    oldOffset = max(_minScrollOffset, oldOffset);
    double newOffset = (oldOffset + (absoluteDelta * (_maxScrollOffset! - _minScrollOffset))).clamp(_minScrollOffset, _maxScrollOffset!);
    return newOffset;
  }
  ...
  void _onTrackTappedOrDragged(Offset position) {
    ...
    final newOffset = (((_maxScrollOffset! - _minScrollOffset) * absoluteY) + _minScrollOffset).clamp(_minScrollOffset, _maxScrollOffset!);
    _actualScrollViewOffset = newOffset;
  ...
  void moveScrollViewToOffset(double newOffset) {
    if (widget.isFixedScroll && _fixedItemLength != null) {
      final closestTopItemOffset = (newOffset ~/ _fixedItemLength!) * _fixedItemLength! + (_minScrollOffset % _fixedItemLength!);
      final closestBottomItemOffset = (newOffset ~/ _fixedItemLength! + 1) * _fixedItemLength! + (_minScrollOffset % _fixedItemLength!);
      ...
      final newScrollControllerOffset = closest(newOffset, closestTopItemOffset, closestBottomItemOffset).clamp(_minScrollOffset, _maxScrollOffset!);
